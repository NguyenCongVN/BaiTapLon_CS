...
namespace BaiTapLon_CS
{
     public partial class SearchOrder : Form
     {
          ...
          private void btnSearch_Click_1(object sender, EventArgs e)
          {
               ...
               "SELECT * FROM (SELECT inv.Remind,cu.Name_Customer,cu.Sex,cu.Address,cu.Birthday,cu.Phone,inv.ID_Invoice,inv.ID_Customer,inv.Diagnostic,inv.Time_Of_Purchase,inv.ID_Manager,ROW_NUMBER() OVER (ORDER BY inv.ID_Invoice ASC) as RowNumber " +
               "FROM Customer AS cu JOIN Invoice AS inv ON " + Subquery + " and cu.ID_Customer = inv.ID_Customer) as temp WHERE temp.RowNumber BETWEEN @startRow AND @endRow END";
               if (isEnter == false)
               {
               ...
               else
               {
                    ...
                    query = "EXEC dbo.searchInvoice @pageNumber ="+page+",@pageSize =" + pageSize;
        ...
        private void bunifuDgv_Click(object sender, EventArgs e)
        {
               int row = bunifuDgv.CurrentCell.RowIndex;
               idInvoice = int.Parse(bunifuDgv.Rows[row].Cells[6].Value.ToString());
               Search_Order_Detail search = new Search_Order_Detail();
               search.Show();
        }
     }
}
